"""
AI Agent Pro - Advanced AI Agent with Memory & Context Awareness
Features:
...
from backend.vector_memory import vector_memory_manager
...
app = FastAPI(
    title="AI Agent Pro",
    description="Advanced AI Agent with Memory, Context Awareness & Document Q&A",
    version="4.0.0"
)
...
class AIClient:
    ...
    async def generate(self, prompt: str, model: str = "auto") -> dict:
        ...
        try:
            # Try OpenAI first
            if openai and self.openai_key and model in ["auto", "openai"]:
                try:
...
async def chat(request: ChatRequest):
    """Chat with AI with optional web search and insights - WITH MEMORY"""
    try:
        response_data = {
        ...
        recent_history = memory_manager.get_conversation_history(limit=5)
        
        # Build context-aware prompt
        context_prompt = request.question
        if context.get("last_topic"):
            context_prompt += f"\n[Context: Previously discussed: {context.get('last_topic')}]"
        
        # Main AI response with context awareness
        chat_response = await ai_client.generate(context_prompt)
        ...
        await vector_memory_manager.save_memory(request.question, "chat", {"role": "user"})
        await vector_memory_manager.save_memory(chat_response["text"], "chat", {"role": "assistant"})
        # Store in memory
        memory_manager.add_conversation(request.question, chat_response["text"], {
        ...
        if request.use_insights:
            ...
            await vector_memory_manager.save_memory(insights["insight"], "insight", {"topic": request.question})
            # Store insight in memory
            memory_manager.add_insight(request.question, insights["insight"], "light")
        ...
        response_data["memory"] = {
            "conversations": len(memory_manager.get_all_conversations()),
            "documents": len(memory_manager.get_documents()),
            "insights": len(memory_manager.get_insights())
        }
...
async def upload_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Save document to Vector Memory for semantic search
        await vector_memory_manager.save_memory(result.get("content", ""), "document", {"filename": file.filename})
...
async def memory_summary():
    ...
    return JSONResponse(content=memory_manager.get_memory_summary())
...
async def get_stored_documents():
    """Get stored documents list"""
    documents = memory_manager.get_documents()
...
async def get_memory_context():
    """Get current context awareness"""
    context = memory_manager.get_context()
    return JSONResponse(content=context)


@app.post("/memory/clear-conversations")
async def clear_conversations():
    """Clear conversation history"""
    memory_manager.clear_conversations()
    return JSONResponse(content={"status": "conversations cleared"})
...
async def clear_all_memory():
    ...
    return JSONResponse(content={"status": "all memory cleared"})
...
async def delete_document(doc_id: str):
    """Delete specific document from memory"""
    success = memory_manager.remove_document(doc_id)
    return JSONResponse(content={"deleted": success})


@app.post("/memory/search")
async def search_memory(query: str):
    """Search through all stored conversations"""
    results = memory_manager.search_conversations(query)
    return JSONResponse(content={"results": results})


# ============================================================================
# "HOW IT WORKS" PIPELINE ENDPOINT
# ============================================================================
...
@app.post("/how-it-works/demo")
async def pipeline_demo(request: PipelineDemoRequest):
    """Endpoint to demonstrate the internal RAG and Embedding Pipeline"""
    try:
        start_time = time.time()
        
        # Step 1: Embedding Generation
        embedder = get_embedding_service()
        vector = embedder.generate_embedding(request.query)
        embedding_slice = vector[:5] # Send only first 5 dimensions for UI display
        
        # Step 2: Vector Search
        retrieved_context = await vector_memory_manager.retrieve_context(request.query, top_k=request.request.top_k)
        
        # Step 3: LLM Generation
        context_str = "\n\n".join([f"[{r['metadata'].get('memory_type', 'context')}] {r['text']}" for r in retrieved_context])
        llm_prompt = f"Context:\n{context_str}\n\nQuestion: {request.query}"
        
        final_response = await ai_client.generate(llm_prompt)
        
        return JSONResponse(content={
            "query": request.query,
            "embedding_sample": embedding_slice,
